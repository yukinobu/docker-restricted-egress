...
`reload` で変更できるのは `BLOCK_CIDRS` です。`NETWORK`・`BRIDGE`・`SUBNET`・`CHAIN` は初回の構成作成時に使用するもので、作成後の変更には後述の「ネットワーク構成を変更する」を参照ください。サービスの設定適用時にこれらの変更を検出した場合は構成を移行せず、エラーとなり変更前のネットワークの一時遮断を維持します。
...
本バージョンには、DNS resolver の例外許可を設定する機能はありません。例外が必要な場合は、管理者が宛先の DNS resolver と UDP/TCP 53 番ポートに限定した追加のファイアウォール設定を別途管理してください。

docker-restricted-egress の専用 chain は `reload` などで再構築されるため、そこへの手動追加は保持されません。追加設定は docker-restricted-egress のルールと適用順序を含めて調整し、起動時・停止中・適用失敗時の一時遮断を迂回させないようにしてください。
